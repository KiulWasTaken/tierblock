...
public class FarmingListener implements Listener {
  ...
  public void cropBreakListener(BlockBreakEvent event) {
    ...
    if (
      user.getIslandAtPosition().getRank(user.getUUID()) <
    ...
    if (
      type.isNether && !user.getStats().getBoolean("farming.nether.unlocked")
    ) {
      user.sendActionBar(
    ...
    if (
      user.getLevel(SkillType.FARMING, type.isNether) < type.levelRequirement
    ) {
      ...
      if (
        type == CropType.CHORUS_FLOWER || type == CropType.NETHER_WART
      ) return; // no progress checks for chorus fruits, no farming goal found.
      boolean progress =
        (type.levelRequirement - 1) ==
        user.getLevel(SkillType.FARMING, type.isNether);
  ...
  public void blockPlaceEvent(BlockPlaceEvent event) {
    ...
    if (block.getType() != Material.BEEHIVE) {
      if (flaggableBlocks.contains(block.getType())) block.setMetadata(
        "pp",
        new FixedMetadataValue(Main.getInstance(), "f")
      );
      return;
    }
    ...
    if (
      user.getIsland().getMetaData().get().get("hasBeeHive").asBoolean() == true
    ) {
    ...
    user
      .getPlayer()
      .playSound(user.getLocation(), Sound.ENTITY_ILLUSIONER_CAST_SPELL, 1, 1);
  }
  ...
  public void harvestBeeHive(PlayerInteractEvent event) {
    ...
    if (block.getType() != Material.BEEHIVE) {
      if (
        ...
        Block above = block
          .getLocation()
          .getWorld()
          .getBlockAt(block.getLocation().add(0, 1, 0));
    ...
    if (
      user.getStats().getBoolean("farming.beehive.booster_active") ||
      user.getBooster() > 1
    ) {
